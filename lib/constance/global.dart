...
class AnimatedForwardArrow extends StatelessWidget {
  ...
  AnimatedForwardArrow(
      {Key key, this.isShowingarroeForward, this.isContrast = false})
      : super(key: key);
  @override
  Widget build(BuildContext context) {
    return Animator(
      ...
      builder: (anim) => FractionalTranslation(
        ...
        child: isShowingarroeForward
            ? Icon(
                ...
                color:
                    AllCoustomTheme.getTextThemeColors(isContrast: isContrast),
                size: 18,
              )
            : SizedBox(),
      ),
...
class AnimatedDivider extends StatelessWidget {
  bool isWithOpacity;
  AnimatedDivider({this.isWithOpacity = false});
  @override
  Widget build(BuildContext context) {
    return Animator(
      ...
      builder: (anim) => SizeTransition(
        ...
        child: Divider(
          thickness: 1,
          color: isWithOpacity
              ? ConstanceData.primaryBlue.withOpacity(0.3)
              : Colors.white,
        ),
      ),
    );
